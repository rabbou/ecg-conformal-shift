...
| C-19 | WHEN the supervised baseline is trained on PTB-XL folds 1–8 and scored on fold 10, ITS MI AUROC SHALL lie within 0.03 of the value the PTB-XL benchmark reports for that split (Strodthoff et al. 2020, IEEE JBHI) — the reference value is read from the paper or its repository before the run, never from memory. | pending — `test_baseline.py` |
